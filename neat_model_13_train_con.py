...
def data_retrieve(data_inputss,index):
    ...
    data_inputss[index] = data_inputs1
...
# Create thread1
thread1 = threading.Thread(target=cont_data)
...
def evaluate_genome(genomes, config, save_path="best_genome2.pkl", winrate_weight=0.6, pnl_weight=0.4):
    ...
    for genome_id, genome in genomes:
        ...
        try:
            ...
            env = TradingEnvironment(data_inputss[0])
            env.reset()
...
def run_neat(config_path):
    ...
    winner = p.run(evaluate_genome, 9999999)
    print('\nBest genome:\n{!s}'.format(winner))
...
if __name__ == "__main__":
    ...
    thread2 = threading.Thread(target=run_neat, args=(config_path,))
    thread2.start()
